...
" Environment {
    ...
    " Vundle {
        ...
        set rtp+=$VIMHOME/hwd-vim/bundle/vundle
        call vundle#rc()
        Bundle 'EvanDotPro/vundle', 'feature/refactor-git-treeish-support', {'local': 1}
    " }

    " Local Configuration {
        if filereadable(expand("$VIMHOME/hwd-vim/config.local.vim"))
            source $VIMHOME/hwd-vim/config.local.vim
        endif
    " }
...
" Bundles {
    ...
    if !exists('g:bundle_groups')
        let g:bundle_groups=['general', 'themes', 'programming', 'php', 'javascript', 'docker']
    endif
...
" Vim UI {
    ...
    set termguicolors
    set background=dark
    silent color material-theme                    " Vim colorscheme
